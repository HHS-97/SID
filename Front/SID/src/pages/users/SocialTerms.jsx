...
- 회원 가입 시 본 약관에 동의한 것으로 간주되며, 서비스 이용 중 약관 내용에 대한 변경이 있을 경우 공지사항 등을 통해 안내합니다.`,
	...
	const handleSubmit = () => {
		if (allowPersonalTerms && allowServiceTerms) {
			axios
				.post(import.meta.env.VITE_BASE_URL + "/social/signup", null, {
					params: {
				...
				.then((response) => {
					// response.data로 실제 응답 데이터 확인
					if (response.data === "Okay") {
						Swal.fire({
							title: "회원가입 성공",
							text: `${userInfo.provider === "kakao" ? "카카오로 " : userInfo.provider === "naver" ? "네이버로 " : ""}로그인을 시도해주세요.`,
							icon: "success",
				...
				<div style={{ textAlign: "end" }}>
					<input
						...
						onChange={(e) => {
							...
							setAllowPersonalTerms(isChecked)
							//console.log("개인정보 수집 및 이용 동의 체크박스:", isChecked) // 상태 로그
						}}
					/>
					<span className="ms-3">개인정보 수집 및 이용에 동의합니다.</span>
				</div>
			</div>
			<div className="mb-4">
				...
				<div style={{ textAlign: "end" }}>
					<input
						...
						onChange={(e) => {
							const isChecked = e.target.checked
							setAllowServiceTerms(isChecked)
							//console.log("이용 약관 동의 체크박스:", isChecked) // 상태 로그
						}}
					/>
					<span className="ms-3">이용 약관에 동의합니다.</span>
				</div>
			</div>
