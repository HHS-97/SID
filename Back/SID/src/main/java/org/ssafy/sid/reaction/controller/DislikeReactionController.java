...
import org.ssafy.sid.reaction.dto.CommentDislikeReactionsDTO;
import org.ssafy.sid.reaction.dto.CommentLikeReactionsDTO;
import org.ssafy.sid.reaction.dto.PostDislikeReactionsDTO;
...
public class DislikeReactionController {
	...
	@PostMapping("/posts")
	public ResponseEntity<?> postsDislike(@RequestBody PostDislikeReactionsDTO postDislikeReactionsDTO, HttpServletRequest request, HttpServletResponse response) {
		...
		Optional<Posts> posts = postsRepository.findById(postDislikeReactionsDTO.getPostId());
		if (posts.isEmpty()) {
		...
		if (postReactions.isPresent()) {
			if (!postDislikeReactionsDTO.getIsDislike() && postReactions.get().getPositive()) {
				// 저장된 데이터가 있으면 update로
				postReaction = reactionsServiceImpl.updatePostDislikeReactions(postReactions.get());
			} else {
				errorResultMap.put("error", "데이터가 잘못됐습니다.");
				return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResultMap);
			}
		} else {
			if (!postDislikeReactionsDTO.getIsDislike()) {
				// 저장된 데이터가 없으면 create로
				postReaction = reactionsServiceImpl.createPostDislikeReactions(posts.get(), profiles);
			} else {
				errorResultMap.put("error", "데이터가 잘못됐습니다.");
				return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResultMap);
			}
	...
	@DeleteMapping("/posts")
	public ResponseEntity<?> deletePostsDislike(@RequestBody PostDislikeReactionsDTO postDislikeReactionsDTO, HttpServletRequest request, HttpServletResponse response) {
		Map<String, Object> resultMap = new HashMap<String, Object>();
		Map<String, Object> errorResultMap = new HashMap<String, Object>();
	...
	public ResponseEntity<?> commentsDislike(@RequestBody CommentDislikeReactionsDTO commentDislikeReactionsDTO, HttpServletRequest request, HttpServletResponse response) {
		Map<String, Object> resultMap = new HashMap<String, Object>();
		Map<String, Object> errorResultMap = new HashMap<String, Object>();
		...
		Optional<Comments> comments = commentsRepository.findById(commentDislikeReactionsDTO.getPostId());
		if (comments.isEmpty()) {
		...
		if (commentReactions.isPresent()) {
			if (!commentDislikeReactionsDTO.getIsDislike() && commentReactions.get().getPositive()) {
				// 저장된 데이터가 있으면 update로
				commentReaction = reactionsServiceImpl.updateCommentDislikeReactions(commentReactions.get());
			} else {
				errorResultMap.put("error", "데이터가 잘못됐습니다.");
				return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResultMap);
			}
		} else {
			if (!commentDislikeReactionsDTO.getIsDislike()) {
				// 저장된 데이터가 없으면 create로
				commentReaction = reactionsServiceImpl.createCommentDislikeReactions(comments.get(), profiles);
			} else {
				errorResultMap.put("error", "데이터가 잘못됐습니다.");
				return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResultMap);
			}
	...
	public ResponseEntity<?> deleteCommentsDislike(@RequestBody CommentDislikeReactionsDTO commentDislikeReactionsDTO, HttpServletRequest request, HttpServletResponse response) {
		Map<String, Object> resultMap = new HashMap<String, Object>();
		Map<String, Object> errorResultMap = new HashMap<String, Object>();
